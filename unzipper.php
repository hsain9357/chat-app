...
class Unzipper
{
    ...
    public static function extractZipArchive($archive, $destination)
    {
        ...
        if ($zip->open($archive) === true) {
            ...
            if (is_writeable($destination . "/")) {
                ...
                $GLOBALS["status"] = [
                    "error" => "Error: Directory not writeable by webserver.",
                ];
            }
        } else {
            $GLOBALS["status"] = [
                "error" => "Error: Cannot read .zip archive.",
            ];
        }
    }
    ...
    public static function extractGzipFile($archive, $destination)
    {
        ...
        if (file_exists($destination . "/" . $filename)) {
            ...
            if (
                pathinfo($destination . "/" . $filename, PATHINFO_EXTENSION) ==
                "tar"
            ) {
                $phar = new PharData($destination . "/" . $filename);
                if ($phar->extractTo($destination)) {
                    $GLOBALS["status"] = [
                        "success" => "Extracted tar.gz archive successfully.",
                    ];
                    // Delete .tar.
    ...
    public static function extractRarArchive($archive, $destination)
    {
        ...
        if (!class_exists("RarArchive")) {
            $GLOBALS["status"] = [
                "error" =>
                    'Error: Your PHP version does not support .rar archive functionality. <a class="info" href="http://php.net/manual/en/rar.installation.php" target="_blank">How to install RarArchive</a>',
            ];
            return;
        }
        // Check if archive is readable.
        if ($rar = RarArchive::open($archive)) {
            ...
            $GLOBALS["status"] = [
                "error" => "Error: Cannot read .rar archive.",
            ];
        }
    }
}
...
class Zipper
{
    ...
    public static function zipDir($sourcePath, $outZipPath)
    {
        ...
        $GLOBALS["status"] = [
            "success" => "Successfully created archive " . $outZipPath,
        ];
    }
}
?>
